...
//   CHECK_INPUT(img_size);
//   CHECK_INPUT(volume);
//   CHECK_INPUT(weight);

//   integrate_cuda(intrinsic, pose, depth, volume_origin, voxel_size, img_size, volume, weight);
// }

// TORCH_EXTENSION_NAME 会将其自动定义为在setup中为该扩展指定的名称
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  //tsdf fusion
  // m.def("integrate", &integrate, "tsdf integrate");
}